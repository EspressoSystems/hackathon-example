...
func ParseL2Transactions(msg *L1IncomingMessage, chainId *big.Int) (types.Transactions, error) {
	...
	case L1MessageType_L2FundedByL1:
		...
		kind := msg.L2msg[0]
		depositRequestId := crypto.Keccak256Hash(msg.Header.RequestId[:], U256Bytes(common.Big0))
		unsignedRequestId := crypto.Keccak256Hash(msg.Header.RequestId[:], U256Bytes(common.Big1))
...
func parseL2Message(rd io.Reader, poster common.Address, timestamp uint64, requestId *common.Hash, chainId *big.Int, depth int) (types.Transactions, error) {
	...
	case L2MessageKind_Batch:
		...
		for {
			...
			if requestId != nil {
				subRequestId := crypto.Keccak256Hash(requestId[:], U256Bytes(index))
...
func parseUnsignedTx(rd io.Reader, poster common.Address, requestId *common.Hash, chainId *big.Int, txKind byte) (*types.Transaction, error) {
	gasLimitHash, err := HashFromReader(rd)
	if err != nil {
		return nil, err
	}
	...
	maxFeePerGas, err := HashFromReader(rd)
	if err != nil {
		return nil, err
	}
	...
	to, err := AddressFrom256FromReader(rd)
	if err != nil {
		return nil, err
	}
...
func parseSubmitRetryableMessage(rd io.Reader, header *L1IncomingMessageHeader, chainId *big.Int) (*types.Transaction, error) {
	retryTo, err := AddressFrom256FromReader(rd)
	if err != nil {
		return nil, err
	}
	pRetryTo := &retryTo
	...
	depositValue, err := HashFromReader(rd)
	if err != nil {
		return nil, err
	}
	maxSubmissionFee, err := HashFromReader(rd)
	if err != nil {
		return nil, err
	}
	feeRefundAddress, err := AddressFrom256FromReader(rd)
	if err != nil {
		return nil, err
	}
	callvalueRefundAddress, err := AddressFrom256FromReader(rd)
	if err != nil {
		return nil, err
	}
	...
	dataLength256, err := HashFromReader(rd)
	if err != nil {
		return nil, err
	}
...
func parseBatchPostingReportMessage(rd io.Reader, chainId *big.Int, msgBatchGasCost *uint64) (*types.Transaction, error) {
	batchTimestamp, batchPosterAddr, _, batchNum, l1BaseFee, extraGas, err := ParseBatchPostingReportMessageFields(rd)
	if err != nil {
		return nil, err
	}
	...
	batchDataGas = SaturatingUAdd(batchDataGas, extraGas)

	data, err := PackInternalTxDataBatchPostingReport(
